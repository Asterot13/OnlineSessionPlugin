	...
	JoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnJoinSessionComplete))
{
	...
	if (OnlineSubystem)
	{
		OnlineSessionInterface = OnlineSubystem->GetSessionInterface();
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Blue,
				FString::Printf(TEXT("Found subsystem - %s"), *OnlineSubystem->GetSubsystemName().ToString())
			);
		}
	}
}
...
void AMenuSystemCharacter::OnFindSessionComplete(bool bWasSuccessful)
{
	...
	if (bWasSuccessful)
	{
		for (auto Findres : SessionSearch->SearchResults)
		{
			FString Id = Findres.GetSessionIdStr();
			FString User = Findres.Session.OwningUserName;
			FString MatchType;
			...
			const ULocalPlayer* LocalPlayer = GetWorld()->GetFirstLocalPlayerFromController();
			OnlineSessionInterface->JoinSession(*LocalPlayer->GetPreferredUniqueNetId(), NAME_GameSession, Findres);
		}
	}
	else
	{
